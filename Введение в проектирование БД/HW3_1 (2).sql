...
INDEX news_category_idx(news_category)
...
CREATE TABLE `music`(
	...
	user_posted_music_id BIGINT UNSIGNED NULL,
	created_at DATETIME DEFAULT NOW(), 
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
INDEX album_artist_idx(music_album,music_artist),
   FOREIGN KEY (user_posted_music_id) REFERENCES users(id)   -- правильно ли я связалал пользователя который выложил музыку у себя в альбом?
);



DROP TABLE IF EXISTS `online_status`;
CREATE TABLE `online status` (
	`online_user_id` SERIAL PRIMARY KEY NOT NULL,
	`inline status` ENUM('Online', 'Offline'),
	`created_at` DATETIME DEFAULT NOW(), 
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    FOREIGN KEY fk_online_user_id(online_user_id) REFERENCES users(id)
